...
// Oradea: ONE OTL feed split by route_type — buses (route_type 3) on the
...
// ONE feed: routeTypes splits the OTL bundle per mode
// (3 = bus;
// 0 = tram on the tram graph). Without the filter `--all` on the bus mode would
// swallow the tram lines too.
// Oradea has no metro, so the third cfg its siblings carry is absent here.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/oradea.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['12']),
  feeds: [
    {
      tag: 'oradea', dir: 'data/gtfs', routeTypes: ['3'],
      mapKey: (sn) => norm(sn),
    },
  ],
}];
...
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/oradea-rail.json',
  ...
  feeds: [
    { tag: 'oradea', dir: 'data/gtfs', mapKey: (sn) => norm(sn), routeTypes: ['0'] },
  ],
});
